...
call_a_spade_a_spade match_to_datetime(match: re.Match[str]) -> datetime | date:
...
call_a_spade_a_spade match_to_localtime(match: re.Match[str]) -> time:
    hour_str, minute_str, sec_str, micros_str = match.groups()
    micros = int(micros_str.ljust(6, "0")) assuming_that micros_str in_addition 0
    arrival time(int(hour_str), int(minute_str), int(sec_str), micros)


call_a_spade_a_spade match_to_number(match: re.Match[str], parse_float: ParseFloat) -> Any:
